...
Кнопка восстановления не занимает место в исходной форме. Она появляется только
после отклонённой попытки входа (`M_FORBIDDEN`) как круглая кнопка с вопросом
слева внутри поля пароля. Весь сценарий остаётся внутри Orex:
...
В `Настройки → Аккаунт → Почта` пользователь может:

- увидеть подтверждённый адрес;
- привязать первый адрес;
- перепривязать почту, сначала подтвердив новый адрес и только затем удалив
  старый;
- отвязать адрес с отдельным предупреждением, если он последний.

Для этого Orex использует стандартные Matrix 3PID endpoints compatibility-слоя:
...
Письмо подтверждается одноразовой ссылкой. При завершении привязки Orex проходит
UIA текущим паролем в собственном диалоге. Пароль от почтового ящика приложению
не нужен и нигде не запрашивается.

После загрузки состояния аккаунта Orex показывает единую верхнюю системную
плашку о непривязанной почте. Она имеет меньший приоритет, чем подтверждение
сессии, обновление клиента и рекомендация защиты истории, поэтому плашки не
накладываются друг на друга. Закрытие рекомендации сохраняется отдельно для
сочетания Matrix-пользователь + устройство.
...
## Системные рекомендации безопасности

Верхняя полоса на главном экране теперь одна. Приоритеты текущей реализации:

1. подтверждение текущей Matrix-сессии;
2. доступное обновление;
3. «Защитите историю сообщений»;
4. «Привяжите почту к аккаунту».

Плашка защиты истории появляется только после получения серверного статуса
хранилища ключей и только когда автобэкап не включён. Нажатие ничего не включает
автоматически, а открывает `Настройки → Хранилище ключей`. Закрытие рекомендации
сохраняется отдельно от настройки автобэкапа для текущих пользователя и
устройства.

Плашка почты появляется только после успешной загрузки `account/3pid` и не
показывает ложное предупреждение при сетевой ошибке. Нажатие открывает нативный
экран управления почтой.
...
- `/auth` используется для OAuth/device-flow, а нативные операции аккаунта —
  через MAS `compat`;
