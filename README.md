...
│  ├─ orex_logger.dart               # единая точка dev-логов Orex
...
В проекте есть единая точка dev-логов `OrexLog`. Логи нужны для сложных Matrix-flow:
создание комнат, настройка power-levels каналов, metadata дочерних чатов
супергруппы, preview и join-flow. Не стоит разбрасывать прямой `debugPrint` по UI:
лучше писать через `OrexLog.d(area, message, error)`, чтобы весь шум можно было
выключить одним флагом.

По умолчанию подробные Orex-логи включены в debug-сборке. Отключение:
