...
- добавлена простая фирменная страница `/download/`, которая берёт актуальную stable-сборку из того же update feed и не требует ручного обновления ссылок.
...
* 🟢 **Простая фирменная страница скачивания.** `/download/` использует тот же
  stable update feed, показывает актуальную версию и прямые ссылки на Windows x64,
  Android ARM64/ARMv7 без отдельного ручного списка релизов;
* 🟡 **Корректный lifecycle Web QR-камеры.** При переключении со сканера обратно
  на показ QR и при закрытии QR-входа браузерный camera capture должен полностью
  завершаться без гонок `start/stop`, зависших media tracks и повторного захвата;
