...
#### Контракт обновления из приложения

Обновление не должно зависеть от GitHub API или от HTML-страницы релиза. Web-
сервер Orex публикует небольшой same-origin manifest, а сами сборки лежат в
неизменяемых каталогах конкретных версий:

```text
/updates/stable/manifest.json
/updates/stable/0.4.3+16/orex-windows-x64.exe
/updates/stable/0.4.3+16/orex-android-arm64-v8a.apk
```

Минимальный формат manifest:
...
```json
{
  ...
  "minimum_supported_version": "0.4.2+14",
  "release_notes_url": "/updates/stable/0.4.3+16/notes.html",
  "platforms": {
    "windows-x64": {
      "url": "/updates/stable/0.4.3+16/orex-windows-x64.exe",
      "sha256": "<hex>",
      "size": 123456789
    },
    "android-arm64-v8a": {
      "url": "/updates/stable/0.4.3+16/orex-android-arm64-v8a.apk",
      "sha256": "<hex>",
      "size": 123456789
    }
  },
  "key_id": "orex-release-2026-01",
  "signature": "<base64>"
}
```
...
- проверка запускается неблокирующе после восстановления локальной сессии, затем
  не чаще одного раза в шесть часов; ручная кнопка «Проверить обновления» может
  выполнить проверку сразу;
- запрос использует короткий timeout, `ETag`/`If-None-Match`, backoff и jitter,
  чтобы временная недоступность сайта не мешала запуску мессенджера;
- версия сравнивается как `version + build`, а отклонённое необязательное
  обновление снова показывается только для более новой сборки;
- manifest принимается только по HTTPS с разрешённого origin. Поле `key_id`
  выбирает встроенный публичный ключ, а `signature` содержит Ed25519-подпись
  канонического UTF-8 JSON без самого поля `signature`; скачанный файл отдельно
  проверяется по размеру и SHA-256, а старый manifest не может инициировать
  downgrade;
- обычное обновление всегда требует явного действия пользователя. Windows
  скачивает installer во временный каталог, проверяет его и только затем
  предлагает запуск; Android использует штатный store/package-installer flow;
  Web активирует подготовленный service worker и предлагает перезагрузить
  вкладку;
- `minimum_supported_version` делает предупреждение недоступным для постоянного
  скрытия, но всё равно не разрешает незаметно заменить исполняемый файл;
- в foreground используется верхняя системная плашка. В background допустимо
  одно системное уведомление на конкретный `version + build`, если пользователь
  разрешил уведомления;
- последний успешно проверенный manifest и причина ошибки сохраняются только для
  диагностики. Нельзя считать отсутствие сети подтверждением, что обновлений нет.
