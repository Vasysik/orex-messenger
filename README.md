...
* 🟢 **Уведомления на Windows.** Аватарки в уведомлениях, отработка
  уведомления при открытом чате, но свёрнутом приложении; сворачивание в трей
* 🟢 **Обновление из приложения.** Реализованы независимые каналы `stable` и
  `debug`, автоматический feed из папок релизов, верхняя плашка, окно с notes и
  прогрессом, а также запуск штатного installer-потока платформы. Сборка и
  публикация описаны в `docs/release-updates.md`;
...
* 🟢 **Демонстрация экрана на Android.** Полный MediaProjection lifecycle:
  системное разрешение, foreground ownership, publication, работа в background
  и безопасная очистка после stop/revoke/lock screen;
...
Реализованная production-схема сводит публикацию релиза к созданию
