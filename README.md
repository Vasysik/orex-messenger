...
5. **Срок хранения на Orex-инфраструктуре.** Настраиваемая политика автоудаления
   серверных копий сообщений/медиа и доступного серверу key-backup материала;
   уже доставленные на пользовательские устройства E2EE-ключи удалённо отозвать
   нельзя, поэтому эта настройка описывает именно Orex-controlled хранение.
6. **Регистрационные коды с происхождением.** Код создаётся конкретным
   пользователем, имеет серверные лимиты/отзыв и сохраняет связь приглашённого с
   создателем кода для будущей relation/trust модели.

Настройки приватности `0.5.0`:

- профиль и личный чат отображают online/last seen только в рамках выбранной
  privacy policy;
...
1. **Media composer и альбомы 2.0.** Улучшенная отправка нескольких медиа,
   предсказуемая группировка/предпросмотр и видимый прогресс загрузки каждого
   элемента без удержания больших файлов целиком в памяти.
2. **Голосовые сообщения.** Запись, предпросмотр, отправка, waveform и
   воспроизведение внутри чата.
3. **Видео-заметки любых форм.** Кружки, квадратики, треугольники, ромбы и
...
5. **Собственный кроссплатформенный мультимедийный плеер.** Единый Orex player
