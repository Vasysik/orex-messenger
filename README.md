...
декодированная аватарка отправителя из локального кэша; если её нет или файл
некорректен, Windows показывает обычную иконку Orex. Свернуть или закрыть окно
...
- страница `/download/` теперь является частью того же Flutter UI, использует фирменные `AmbientBackground` / `GlassPanel` / brand-компоненты и тот же stable update feed; переход к ней встроен в Web auth и панель чатов;
...
- Web-аватары получили persistent CacheStorage; брендовые asset-изображения кешируются nginx, а иконка Orex появляется мягко без временной картинки-заглушки.
...
* 🟢 **Фирменная Flutter-страница скачивания.** `/download/` открывает тот же
  ...
  вручную. В Web переход встроен в экран входа и шапку списка чатов без отдельной
  плавающей кнопки;
...
* 🟡 **Медиа-плитки звонка без изменения размера контента при speaking.** Если
  плитка показывает видео или screen share, speaking-анимация работает внешним
  акцентом и не уменьшает само медиа; avatar-only плитки могут сохранять текущий
  scale-эффект;
* 🟡 **Полноэкранный режим выбранной медиа-плитки.** Уже существующий focused
  ...
  предыдущий режим, call controls доступны поверх fullscreen;
...
* 🟡 **Разделители дней в timeline.** Между сообщениями разных календарных дней
  показывать компактную метку даты в стиле системной карточки входа в чат, но без
  обводки, чтобы сообщения с одинаковым временем в разные дни не выглядели одной
  непрерывной группой;
