...
* 🟡 **Уведомления на Windows.** Закрытый или завершённый desktop-клиент должен
  получать remote-push, показывать нативное уведомление и по нажатию безопасно
  запускаться с переходом в нужный чат; текущие local sync-уведомления при
  живом процессе — только первый этап;
