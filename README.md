...
Текущий stable-релиз — **0.4.4+7**. История стабильных пользовательских
обновлений ведётся в [`CHANGELOG.md`](CHANGELOG.md); debug-сборки туда не
включаются. Ветка `0.4.x` уже рассматривает как базовые функции нативную
авторизацию Synapse, системные Android-звонки, killed-process push,
...
быть заново подтверждены на Android, Web и Windows. Stable-канал не является
заявлением о независимой security-сертификации: для этого нужен отдельный аудит.
...
**0.4.4+7** завершает текущий stable hardening. Уже выпущенные изменения не
дублируются в roadmap: история stable-релизов находится в
[`CHANGELOG.md`](CHANGELOG.md), а ниже остаются только будущие задачи.

Отдельным техническим долгом остаются миграция Android/Flutter plugins на
Built-in Kotlin, server-side enforcement LiveKit grants/voice permissions,
переход на `sqlite3 3.x` после совместимого Matrix SDK, следующий major
`flutter_secure_storage` и дальнейшее дробление `VoipService` только по
подтверждённым runtime-проблемам.

Древо разработки ниже фиксирует продуктовый порядок работ и зависимости между
функциями. Оно не превращает каждый пункт в обещание конкретной даты: версии —
...
* 🟡 **Устаревший входящий звонок после запуска.** После старта или возврата Orex
  не должен показывать входящий вызов по старой попытке, если на вызывающем
  устройстве звонок уже давно завершён. Нужно проверить согласование ring-события,
  точного идентификатора попытки, MatrixRTC membership и cold-start push, чтобы
  устаревшая попытка не восстанавливалась как живая, но реально продолжающийся
  входящий звонок не терялся;
* 🟡 **Уход удалённого участника.** Если собеседник завершил звонок, его плитка и
  presence должны исчезнуть без дополнительного локального действия. Нужно
  проверить отставание MatrixRTC membership и локального roster при remote hangup
  и reconnect, чтобы после фактического выхода не оставался «призрачный»
  участник;
...
* 🟡 **Privacy-safe crash reporting и диагностика критических flow.** Добавить
