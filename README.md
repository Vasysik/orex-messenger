...
- страница `/download/` теперь является частью того же Flutter UI, использует фирменные `AmbientBackground` / `GlassPanel` / brand-компоненты и тот же stable update feed; на Web к ней ведёт глобальная круглая кнопка в левом нижнем углу всех основных экранов и меню (кроме самой `/download/`), оформленная тем же медным градиентом, что и кнопка отправки сообщения, и чуть более заметной тёмной обводкой; hover не использует `Tooltip`/`Overlay`, поэтому кнопка безопасно живёт в глобальном `MaterialApp.builder`;
...
* 🟢 **Фирменная Flutter-страница скачивания.** `/download/` открывает тот же
  ...
  Глобальная кнопка не использует `Tooltip`/`Overlay`, поэтому hover безопасен и
  не зависит от положения `Navigator` внутри `MaterialApp.builder`;
