...
attachment queue и timeline grouping. Timeline также вставляет разделители
календарных дней и поддерживает sticky-date: обычная метка дня, подходя к верхней
границе ленты, визуально отрывается от своего места и закрепляется там без второго
дубликата. Переход между днями учитывает верхнюю и нижнюю границы реальной pill,
поэтому соседние даты не пересекаются даже на 1–2 px; при обратной прокрутке метка
снова присоединяется к разделителю.
...
- fullscreen выбранной media-плитки автоматически уводит нижние call controls
  вниз после 3 секунд бездействия; тап/клик по экрану возвращает их и заново
  запускает таймер скрытия;
...
- timeline разделяет сообщения по локальным календарным дням; sticky-date использует тот же стиль, что и обычный разделитель, переключается только после прохождения полной высоты pill и проверяет пересечение по её верхней и нижней координатам, поэтому соседние даты не накладываются; focused media-плитка звонка поддерживает отдельный fullscreen с возвратом по double tap, `Esc` и системному Back, speaking-рамка не меняет размер видео/screen share, а нижние fullscreen call controls автоматически скрываются вниз после 3 секунд бездействия и возвращаются по тапу/клику.
...
* 🟢 **Полноэкранный режим выбранной медиа-плитки.** Уже существующий focused
  ...
  предыдущий режим. Нижние call controls доступны поверх fullscreen, через 3 секунды
  бездействия плавно уезжают вниз и возвращаются по тапу/клику с перезапуском таймера;
...
* 🟢 **Разделители и sticky-date в timeline.** Между сообщениями разных
  ...
  `bottom`), поэтому соседние даты не пересекаются при субпиксельном скролле, а
  при обратной прокрутке метка снова присоединяется к разделителю;
